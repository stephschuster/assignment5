...
public class GUI extends JFrame{
    ...
    JRadioButtonMenuItem numberOfPoints, getPoints, averagePoints, medianPoints, addPoint;//, halfPlaneX, halfPlaneY;
...
/////////////////////////////////////////////////////////////////////////
    private void jbInit() throws Exception {
        ...
        numberOfPoints = new JRadioButtonMenuItem("numberOfPointsInRange");
        numberOfPoints.setSelected(true);
        group.add(numberOfPoints);
        workingModes.add(numberOfPoints);
        getPoints = new JRadioButtonMenuItem("getPointsInRange");
        group.add(getPoints);
        ...
        //***
        averagePoints = new JRadioButtonMenuItem("averageHeightInRange");
        group.add(averagePoints);
        workingModes.add(averagePoints);
        medianPoints = new JRadioButtonMenuItem("getMedianPoints");
        group.add(medianPoints);
        workingModes.add(medianPoints);
        //***
    ...
    void loadTXTMenuItem_actionPerformed(ActionEvent e) {
		try {
        	...
        	if (returnVal == JFileChooser.APPROVE_OPTION || withOutLoading) {
                ...
                medianY=points[points.length/2];
                Arrays.sort(points,c2);
                pdt=new PointDataStructure(points, medianY);
...
/////////////////////////////////////////////////////////////////////////
	...
	public Point[] getPointsInRange(int XLeft, int XRight)
	{
		...
		Point[] points=pdt.getPointsInRange(XLeft, XRight);//new Point[0];//tree.getPointsInRectangle(new Point(leftTopX,leftTopY),new Point(rightBottomX,rightBottomY));
		return points;//TODO
	...
	{
		this.pdt.addPoint(new Point(X,Y,name));
		String[] ans=new String[1];
...
class PicturePanel extends JPanel implements MouseListener, MouseMotionListener {
    ...
    public void paintComponent(Graphics g) 
    {
        ...
        if (drawingNow && (myGUI.numberOfPoints.isSelected() || myGUI.getPoints.isSelected() || myGUI.averagePoints.isSelected()))
        {
...
//        	g2d.drawLine(0, currY, 1000, currY);
        ...
        if (drawNames && myGUI.medianPoints.isSelected())
        {
    ...
    {
    	if  (myGUI.numberOfPoints.isSelected() || myGUI.getPoints.isSelected() || myGUI.averagePoints.isSelected())
    	{
    		...
    		{
    			points=myGUI.getPointsInRange(Math.min(startX,tmpX),Math.max(startX,tmpX));
    			String[] ans;
...
//    					Math.min(startY,tmpY),Math.max(startY,tmpY)));
    ...
    public void mouseDragged(MouseEvent e) {drawAverageLine=false;repaint(); if (myGUI.numberOfPoints.isSelected() || myGUI.getPoints.isSelected() || myGUI.averagePoints.isSelected()){tmpX = e.getX();tmpY = e.getY();}}
